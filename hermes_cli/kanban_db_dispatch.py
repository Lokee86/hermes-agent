...
from hermes_cli.quiet_single_query import KANBAN_WORKER_EXIT_RUN_TRAILER
...
_RUN_EXIT_TRAILER_RE = re.compile(
    r"^" + re.escape(KANBAN_WORKER_EXIT_RUN_TRAILER) + r"(\d+) rc=(\d+)\s*$", re.MULTILINE,
)
...
def _worker_log_exit_code(
    ...
    *,
    ...
    """Exit code from the durable trailer for this worker run; None when absent.

    Logs are task-scoped and append across retries, while an exit classification belongs to one
    ``task_runs.id``. When ``run_id`` is known, only a run-qualified trailer for that exact
    attempt is authoritative; an older attempt's legacy or qualified trailer must never leak
    forward into a killed/OOMed run that wrote no trailer of its own. Without a run id, retain
    the legacy last-trailer behaviour for pre-run-id rows and old logs.
    """
    try:
    ...
    if run_id is not None:
        matches = _RUN_EXIT_TRAILER_RE.findall(text)
        for logged_run_id, code in reversed(matches):
            if int(logged_run_id) == int(run_id):
                return int(code)
        return None
    # Compatibility only: callers without a run identity may consume either old or new receipts.
    # Preserve textual ordering when both formats are present rather than preferring one generation.
    candidates: list[tuple[int, int]] = []
    candidates.extend((m.start(), int(m.group(2))) for m in _RUN_EXIT_TRAILER_RE.finditer(text))
    candidates.extend((m.start(), int(m.group(1))) for m in _EXIT_TRAILER_RE.finditer(text))
    if not candidates:
        return None
...
def _classify_dead_worker(
    ...
    *,
    ...
    """
    dead = _classify_dead_worker_exit(pid, claimer, task_id=task_id, run_id=run_id, board=board)
...
def _classify_dead_worker_exit(
    ...
    *,
    ...
    if kind == "unknown" and task_id:
        logged = _worker_log_exit_code(task_id, board=board, run_id=run_id)
