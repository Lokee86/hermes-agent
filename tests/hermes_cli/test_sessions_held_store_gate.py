...
def _optimize(force=False):
    return Namespace(sessions_action="optimize", force=force)


def test_optimize_refuses_and_names_the_holder_until_forced(state_db, foreign_holder, capsys):
    assert sessions_cmd.cmd_sessions(_optimize()) == 1
    out = capsys.readouterr().out
    assert f"PID {foreign_holder.pid} (" in out
    assert "Refusing `hermes sessions optimize`" in out and "--force" in out
    assert "Optimized" not in out

    assert sessions_cmd.cmd_sessions(_optimize(force=True)) is None
...
def test_prune_preview_passes_the_delete_waits_for_a_quiet_store(state_db, foreign_holder, capsys):
    ...
    prune_preview = Namespace(sessions_action="prune", dry_run=True, yes=False, force=False, never_active=False,
